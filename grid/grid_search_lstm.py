...
for i, params in enumerate(param_combinations):
    ...
    for repeat in range(N_REPEATS):
        ...
        best_model_path = f"checkpoints/best_model_param_{i}_repeat_{repeat}.pth"
...
for i in range(N_REPEATS):
    ...
    best_test_smoothl1 = float('inf')
    best_model_path = f"checkpoints/best_final_model_exp{i+1}.pth"
    ...
    plot_prediction_curve(
        ...
        title=f"Prediction vs Ground Truth (Sample {sample_id}) [Inverse]"
    )
    ...
    plot_loss_curves(
        train_losses,
        test_smoothl1_losses,
        save_path=f"plots_best/loss_curve_exp{i+1}.png"
    )
...
plot_prediction_curve(
    ...
    output_len=OUTPUT_LEN,
    save_path=f"plots_best/avg_prediction_sample{best_sample_id}.png",
    title=f"Avg Prediction over 5 Runs (Sample {best_sample_id})"
)
...
for i in range(N_REPEATS):
    ...
    plot_prediction_curve(
        ...
        y_pred,
        output_len=OUTPUT_LEN,
        save_path=f"plots_best/best_sample{best_sample_id}_exp{i+1}.png",
        title=f"Sample {best_sample_id} - Prediction in Run {i+1}"
    )